# Vendored from macos-fuse-t/homebrew-cask (Casks/fuse-t.rb @ b875611a) so
# mesa can pin the FUSE-T version its releases are tested against instead of
# tracking upstream's HEAD — FUSE-T behavior (xattr/namedattr handling in
# particular) has changed between releases.
#
# To upgrade: validate the new FUSE-T version against mesa's macOS mount
# (xattr sidecar e2e test + manual namedattr probe), then copy the version,
# sha256, and any changed stanzas from upstream's cask.
cask "fuse-t" do
  version "1.0.54"
